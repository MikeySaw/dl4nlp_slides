...
\item Calculate the number of parameters in transformer architectures
\item Understand the required computation and memory in transformers
...
% \begin{itemize}
%     \item FLOPs-seconds which is [Floating Point Ops / Second]  x [Seconds]
% 	\begin{itemize}
...
\begin{itemize}
	...
	\item For models used in production: First determine the acceptable maximum inference cost, and then train the biggest model within that budget.
\end{itemize}
...
AdamW: $Mem_{\text{optimizer}} = 12 ~bytes/param \cdot N_{params}$
\begin{itemize}
...
bitsandbytes (8-bit optimizer): $Mem_{\text{optimizer}} = 6 ~bytes/param \cdot N_{params}$
\begin{itemize}
 	\item fp32 copy of parameters: 4 bytes/param
...
\begin{itemize}
	...
	\end{itemize}
    \item \textit{Efficiency:} Are there more efficient (e.g., linear) implementations that achieve similar or even better performance?
\end{itemize}
